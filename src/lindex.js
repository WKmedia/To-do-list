...
const detailInput = new ElementCreator();
const dateInput = new ElementCreator();
const select = new ElementCreator();
const optionLow = new ElementCreator();
const optionMedium = new ElementCreator();
const optionHigh = new ElementCreator();
...
class CreateNewToDoPopup {
  ...
  createPopUp() {
    ...
    this.createNewPopUp.appendChild(this.mainCreateNew);
...
class ToDo {
  constructor(priority, title, details, date, notes='', checklist= []) {
...
function displayTodo(todo) {
  ...
  listItem.innerHTML = `<strong>${todo.title}</strong> - ${todo.details} - Due: ${todo.dueDate} - Priority: ${todo.priority}`;
...
class ToDoList {
  constructor() {
    this.projects = [new Project('Defualt Project')];
    this.currentProject = this.project[0];
  }
